...
---`;
  ...
  const systemPrompt = savedSystemPrompt || `
        ...
        [핵심 원칙]
        1. **100% 누락 없는 망라**: 전달된 DB 내의 모든(EVERY) 개념을 반드시 가이드 포함하십시오.
        2. **압축 데이터의 핵심 복원**: 입력을 위해 압축된 데이터(D:...)를 바탕으로, 고도의 전문 지식을 활용하여 **핵심 위주의 정교한 설명**을 제공하십시오.
        3. **분량 최적화 (Concise)**: 각 개념 설명은 중학생~고등학생이 읽기에 가장 적절한 **3-4문장** 내외로 압축하여 전달하십시오. 불필요한 서술을 줄여 API 효율성을 극대화합니다.
        4. **완벽한 카테고리 맵핑**: 제공된 데이터를 [시, 형태소, 품사, 문법, 문장구조, 음운] 범주에 맞게 엄격히 분류하십시오. 범주가 불분명할 경우 제목/키워드를 기반으로 합리적으로 배치하십시오.
        5. **문장 종결 어미**: 반드시 "~입니다.", "~하는 현상입니다."와 같은 완전한 문장으로 끝맺으십시오.
    `;
  ...
  const userPrompt = `
        ...
        [작성 지침]
        1. **전체 포함**: 데이터베이스에 있는 모든 개념을 각각의 제목으로 하여 가이드에 포함하십시오.
        2. **핵심 요약**: 각 개념의 정의를 작성할 때 장황한 설명을 피하고, 해당 개념만의 핵심 특성과 예시(필요시)를 곁들여 명료하게 기술하십시오.
        3. **카테고리 마스터**: 
           - '시': 화자, 청자, 정서, 심상, 운율, 비유, 상징 등
           - '형태소': 형태소, 어근, 접사, 합성어, 파생어 등
           - '품사': 체언, 용언, 조사, 관형사, 부사 등
           - '문법': 주어, 서술어, 목적어, 보어 등
           - '문장구조': 홑문장, 겹문장, 사동문, 부정문 등
           - '음운': 음절, 자음/모음, 교체, 탈락, 첨가, 축약 등
        4. **구조적 가독성**: 명확한 섹션 구분과 간결한 설명을 통해 한눈에 모든 개념이 들어오게 하십시오.
