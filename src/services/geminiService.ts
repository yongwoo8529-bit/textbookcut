...
---`;
  ...
  const systemPrompt = savedSystemPrompt || `
        ...
        2. **압축 데이터 복원**: 입력을 위해 압축된 데이터(D:...)를 바탕으로, 당신의 풍부한 전문 지식을 활용하여 고품질의 학술적 설명을 **완전한 문장**으로 복원/확장하십시오.
        3. **문장 종결 어미 준수**: "~하는.", "~함."과 같이 끊기는 형태가 아닌, **"~입니다.", "~하는 현상입니다.", "~함을 의미합니다."**와 같이 **완벽하게 끝맺음된 문장**만 사용하십시오.
        4. **카테고리별 분류**: [시, 형태소, 품사, 문법, 문장구조, 음운] 범주로 모든 개념을 정확히 분류하십시오.
        5. **내용의 차별성**: 각 개념은 고유한 핵심 특성을 중심으로 설명하며, 반복적인 템플릿 문장을 지양하십시오.
    `;
  ...
  const userPrompt = `
        ...
        [작성 지침]
        1. **전체 포함 (Full Coverage)**: 데이터베이스에 있는 모든 개념을 각각의 제목으로 하여 가이드에 포함하십시오.
        2. **유니크한 설명**: 각 개념의 정의를 작성할 때 다른 개념과 겹치는 표현을 최소화하고, 해당 개념만의 핵심 특성을 기술하십시오.
        3. **카테고리 맵핑**: 
           - '시': 화자, 청자, 정서, 심상, 운율, 비유 등
           - '형태소': 형태소, 어근, 접사 등
           - '품사': 체언, 용언, 동사, 형용사 등
           - '문법': 주어, 서술어, 목적어 등
           - '문장구조': 홑문장, 겹문장, 안은 문장 등
           - '음운': 음절, 자음/모음, 교체, 탈락, 첨가, 축약 등
        4. **가독성 극대화**: 불렛 포인트와 명확한 개념명을 사용하여 가독성을 높이십시오.
