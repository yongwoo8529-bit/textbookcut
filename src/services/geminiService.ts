...
---`;
  ...
  const systemPrompt = `
    ...
    ============================================================================
    [1단계: 데이터 소스 및 포괄성 규칙 - 매우 중요]
    ============================================================================
    1. **모든 데이터 포함**: 데이터베이스에서 조회된 '모든 핵심 개념'과 '모든 함정 포인트', '모든 그래프/표 패턴'을 하나도 빠짐없이 요약에 포함하십시오. 일부만 선택해서는 안 됩니다. 데이터베이스의 모든 항목을 골고루 다루어야 합니다.
    2. **계산 문제 전수 포함**: '계산형 문제' 섹션에 있는 모든 공식과 계산 단계를 상세히 설명 섹션에 포함하십시오. 학생이 직접 풀어보고 연습할 수 있을 정도로 단계별로 자세히 풀이 과정을 설명하십시오.
    3. **출처 명시**: 각 개념 설명 끝에 '중학교 X학년 과정'임을 명시하여 신뢰도를 높이십시오.
    4. **DB 데이터 외 생성 금지**: 외부 지식을 섞지 말고 오직 하단에 제공된 데이터베이스 결과로만 구성하십시오.
    5. **교육 수준 필터 강제**: 중학교 범위(middle_1, middle_2, middle_3)인 개념만 사용하십시오.
    6. **추측 생성 금지**: DB에 없는 개념을 추측하거나 만들어내지 마십시오.
    7. **고등학교 심화 개념 금지**: 고1 범위를 벗어나는 고급 개념을 생성하지 마십시오.
