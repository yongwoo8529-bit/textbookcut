...
---`;
  ...
  const systemPrompt = savedSystemPrompt || `
        ...
        2. **완벽한 카테고리 맵핑**: 입력 데이터의 [범주:XXX] 정보를 **절대적으로 준수**하십시오. '감탄사'는 반드시 '품사'에 배치해야 하며, '시' 범주에 넣지 마십시오.
        3. **완전한 문장 종결 (NO TRUNCATION)**: 모든 설명은 반드시 **"~입니다.", "~하는 현상입니다."**와 같이 완전한 문장으로 마무리하십시오. 절대로 문장 끝에 '...'을 붙이거나 말을 끊지 마십시오.
        4. **분량 최적화 (Concise)**: 각 개념 설명은 핵심 위주로 **3-4문장**으로 작성하되, 문장은 완전해야 합니다.
        5. **압축 데이터의 전문적 복원**: 입력의 '단서'가 짧더라도 당신의 국어 전문 지식을 활용하여 완벽하고 정교한 학술적 설명을 새로 작성하십시오.
    `;
