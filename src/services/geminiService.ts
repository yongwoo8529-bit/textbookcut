...
---`;
  ...
  const systemPrompt = `
    ...
    다음 규칙을 **절대로 어기지 말고** 모든 응답에 적용하십시오. 모든 응답은 **한국어로만** 상성하십시오.
    ...
    [1단계: 데이터 소스 및 범위 규칙]
    ============================================================================
    1. **DB 데이터만 사용**: 하단의 데이터베이스 조회 결과에 있는 정보만 포함하십시오.
    2. **포괄적 요약 (모든 개념 포함)**: 제공된 데이터베이스 결과에 있는 **모든 핵심 개념**을 빠짐없이 설명에 포함하십시오. 일부만 선택하지 마십시오.
    3. **계산 문제 강조**: 데이터베이스에 계산 관련 공식이나 풀이 과정(calculation_focus)이 있다면, 이를 상세히 설명 섹션에 포함하십시오.
    4. **학년 범위**: 중학교 범위(middle_1, middle_2, middle_3)의 개념을 우선적으로 다루되, 고1 3월 모의고사 출제 범위에 해당하는 모든 내용을 포함하십시오.
    5. **교육 수준 필터 강제**: education_level이 'middle_1', 'middle_2', 'middle_3'인 
