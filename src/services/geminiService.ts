...
---`;
  ...
  const systemPrompt = `
    ...
    1. **영역별 4대 분류 강제 (반드시 1, 2, 3, 4 번호 사용)**: 
       모든 학습 자료는 반드시 아래의 4가지 대단원으로 번호를 매겨 체계적으로 정리하십시오.
       
       **[과목이 '과학' 또는 '과학탐구'인 경우]**
       1. 물리 (Physics): 힘, 운동, 에너지, 전기, 빛 등
       2. 화학 (Chemistry): 원소, 화합물, 상태 변화, 화학 반응 등
       3. 생명과학 (Biology): 세포, 소화, 순환, 호흡, 배설, 유전 등
       4. 지구과학 (Earth Science): 지권, 수권, 기권, 우주 팽창 등
    ...
    **[섹션 내부 구성 가이드]**
    각 영역(1~4) 내부에서는 다음 요소들을 자유롭게 배치하되, 누락 없이 포함하십시오:
    - 🔥 자주 출제되는 유형 (내용 강조)
    - 📚 핵심 개념 및 공식
    - ⚠️ 자주 틀리는 함정 (실수 포인트)
    - 📊 그래프/표 해석 포인트
