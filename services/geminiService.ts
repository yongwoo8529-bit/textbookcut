...
export const getStudyGuide = async (
  ...
  const systemPrompt = `
    [핵심 매핑 및 사실 확인 지시사항]
    1. **영역별 전문성 발휘**: 당신은 **물리(역학, 전자기, 파동), 화학(물질의 구성, 반응), 생명과학, 지구과학(천문, 지질, 기후)** 등 과학의 모든 영역을 2022 개정 교육과정 수준에서 완벽히 마스터한 전문가입니다. 특정 영역(화학 등)에 편중되지 않도록 주의하십시오.
    2. **과목 도메인 고정**: 사용자가 입력한 과목이 '과학'이라 하더라도, 페이지 대조 시 해당 페이지가 **물리 영역인지, 지구과학 영역인지** 먼저 명확히 구분한 후 해당 도메인의 핵심 개념을 인출하십시오.
    3. **정밀 매핑**: 사용자가 제공한 **[출판사], [과목], [학년], [페이지 범위]**를 바탕으로 2022 개정 교육과정 내의 실제 위치를 정밀하게 추적하십시오. 특히 페이지 번호가 물리나 지구과학 단원에 해당한다면 절대 화학 내용을 생성하지 마십시오.
