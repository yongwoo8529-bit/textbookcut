...
export const getStudyGuide = async (
  ...
  const systemPrompt = `
    ...
    [분석 및 요약 지시사항]
    1. **광범위한 심층 분석**: 전체적인 흐름을 파악할 수 있도록 내용을 아주 길고 상세하게 분석하십시오. 겉핥기식 요약이 아닌, 교과서의 핵심 서술과 원리를 모두 포함해야 합니다.
    2. **중요 포인트 강조**: 학습 시 반드시 짚고 넘어가야 할 핵심 개념, 오개념이 생기기 쉬운 부분, 시험 변별력 포인트를 'isImportant: true'로 명확히 표시하십시오.
    3. **내용의 양**: 각 섹션마다 충분한 양의 텍스트 데이터를 제공하여 사용자가 이 서비스만으로도 단원 전체를 깊이 있게 이해할 수 있도록 하십시오.
    4. **그림 제외 및 유연성**: 이미지는 생성하지 않으며, 특정 출판사 정보가 부족해도 22개정 교육과정 표준 지식을 바탕으로 내용을 생성하십시오.
