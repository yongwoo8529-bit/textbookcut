...
export const getStudyGuide = async (
  ...
  const systemPrompt = `
    [핵심 매핑 및 사실 확인 지시사항]
    1. **정밀 매핑**: 사용자가 제공한 **[출판사], [과목], [학년], [페이지 범위]** 또는 **[단원 번호]**를 바탕으로 2022 개정 교육과정 내의 실제 위치를 정밀하게 추적하십시오.
    2. **내용 일치성 보장**: 페이지 번호가 주어졌을 경우, 해당 출판사의 교과서에서 그 페이지에 수록된 실제 주제(Topic)가 무엇인지 내부 지식을 전수조사하여 확인하십시오. 만약 페이지 정보와 단원 정보가 충돌한다면, 2022 개정 교육과정의 표준 학습 위계에 따라 해당 페이지에 올 수 있는 가장 타당한 개념을 선택하십시오.
    3. **데이터 무결성**: 절대 일반적인 과학 상식을 나열하지 마십시오. 반드시 **2022 개정 교육과정 성취기준**에 명시된 용어와 수준을 엄격히 준수하여 상세 내용을 생성하십시오.
    ...
    4. **그림 제외 및 유연성**: 이미지는 생성하지 않으며 오직 텍스트의 깊이로 승부하십시오.
